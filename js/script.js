document.addEventListener("DOMContentLoaded", () => {
  ...
  customSelects.forEach((select) => {
    ...
    if (!trigger || !valueDisplay || !nativeSelect) return;
    ...
    options.forEach((option) => {
      option.addEventListener("click", (e) => {
        ...
        valueDisplay.textContent = text || "Seleccionar";
    ...
    const selectedOption = select.querySelector(`.custom-select-option[data-value="${initialValue}"]`);
    if (selectedOption) {
      ...
      if (firstOption) {
        ...
        valueDisplay.textContent = firstOption.textContent.trim();
      }
    }
  });
...
${descripcion || "Sin descripción adicional."}`;
  ...
  document.querySelectorAll("[data-comparison]").forEach((comparison) => {
    ...
    comparison.addEventListener("pointerdown", (event) => {
      ...
      updateComparison(getPosition(event));
      event.preventDefault();
    });

    comparison.addEventListener("pointermove", (event) => {
      if (!dragging || event.pointerId !== pointerId || event.pointerType !== "mouse") return;
      updateComparison(getPosition(event));
    });
    ...
    control.addEventListener("pointerup", stopMobile);
    control.addEventListener("pointercancel", stopMobile);
  ...
  if (gallery && scrollbar && thumb) {
    ...
    gallery.addEventListener("pointermove", (event) => {
      ...
      const dx = event.clientX - galleryStartX;
      const dy = event.clientY - galleryStartY;
      ...
      gallery.scrollLeft = galleryStartScroll - dx;
